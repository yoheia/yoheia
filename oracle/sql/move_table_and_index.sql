...
define tablespace_for_table = upper('&tablespace_for_table')
define tablespace_for_index = upper('&tablespace_for_index')
...
begin
	for rec in cu loop
		...
		begin
			sql_stmt := 'alter table "'|| rec.owner ||'"."'||rec.table_name ||'" move tablespace &&tablespace_for_table';
			dbms_output.put_line(sql_stmt);
			execute immediate sql_stmt;
		exception when others then
			dbms_output.put_line(rec.owner||'.'||rec.table_name||':[ '|| sqlcode||'] '||sqlerrm);
		...
		begin
			for rec_indx in cu_indx loop
				begin
					sql_stmt := 'alter index "'||rec_indx.owner||'"."'||rec_indx.index_name||'" rebuild tablespace &&tablespace_for_index compress online';
					dbms_output.put_line(sql_stmt);
					execute immediate sql_stmt;
					exception when others then
